...
<html>
	<head>
		<title>School Administrstion Platform (SAP)</title>
			...
			<div id="section">
					...
					<?php
						if(isset($_POST["submit"]))
						{
							...
							$target_file=$target.basename($_FILES["img"]["name"]);
								...
								$sq="insert into student(RNO,SNAME,FNAME,DOB,GEN,PHO,MAIL,ADDR,SCLASS,SSEC,SIMG,TID) values('{$_POST["rno"]}','{$_POST["name"]}','{$_POST["fname"]}','{$edate}','{$_POST["gen"]}','{$_POST["pho"]}','{$_POST["email"]}','{$_POST["addr"]}','{$_POST["cla"]}','{$_POST["sec"]}','{$target_file}','{$_SESSION["TID"]}')";
				...
				<div class="lbox">
					<label> ID</label><br>
						<?php
							$no="S101";
							$sql="select * from student order by SID desc limit 1";
							$res=$db->query($sql);
							if($res->num_rows>0)
							{
